...
class CreateDeclaration:
    def __init__(self, parking_place, date, yacht, owner_details, commissioning_body, parking_peroid, fee, fee_words, chip_card,
                 media):
    ...
    def create_document(self):
        ...
        self.document.add_paragraph('Opłata za postój wynosi: ', style='List Number').add_run(
            str(self.fee['parking_fee']) + ' zł.').bold = True
        self.document.add_paragraph('Słownie: ', style='List Number').add_run(
            str(self.fee_words)).bold = True
        self.document.add_paragraph(
            'płatna, zgodnie z wystawioną fakturą z góry, jednorazowo lub w poniższych ratach za każdy kwartał w '
            'terminie do:')
        self.document.add_paragraph('      I. 15.05.2020, w kwocie: ').add_run(str(self.fee['quarter_fee']) + ' zł;').bold = True
        self.document.add_paragraph('     II. 15.08.2020, w kwocie: ').add_run(str(self.fee['quarter_fee']) + ' zł;').bold = True
        if self.parking_peroid['to'] == '31.04.2021':
            self.document.add_paragraph('    III. 15.11.2020, w kwocie: ').add_run(str(self.fee['quarter_fee']) + ' zł;').bold = True
            self.document.add_paragraph('    IV. 15.02.2021, w kwocie: ').add_run(str(self.fee['quarter_fee']) + ' zł;').bold = True
        ...
        self.document.add_paragraph(
            ...
            'konieczność zmiany miejsca lokalizacji postoju jachtu na inne zgodnie ze wskazaniem '
            'upoważnionego pracownika przystani.\n\n'
            'Oświadczam, iż zapoznałem się z Regulaminem przystani Narodowego Centrum Żeglarstwa AWFiS w '
            'Gdańsku, i deklaruję postój zgodnie ze złożoną deklaracją. W pełni go akceptuję, co poświadczam '
            'własnoręcznym podpisem pod Deklaracją postoju.\n\n').paragraph_format.alignment = 3
        self.document.add_paragraph(".........................................................				 .........."
                                    "...............................................").add_run(
            '   podpis pracownika NCŻ                                                                                 '
            '             podpis (imię, nazwisko)').italic = True

        self.document.save('deklaracje/Deklaracja {}.docx'.format(self.yacht['name']))
